...
// Wire up the trace search effect (query → filter → results)
const teardownSearch = createTraceSearchEffect(store);
const form = document.getElementById("trace-search-form");
const input = document.getElementById("trace-query");
const prompt = document.getElementById("trace-prompt");
const resultsEl = document.getElementById("trace-results");
const loadingEl = resultsEl.querySelector(".region-loading");
const contentEl = resultsEl.querySelector(".region-content");
...
const unsub = store.subscribe(TOPICS.TRACE_RESULTS, (payload) => {
