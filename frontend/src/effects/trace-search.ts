...
function matchesQuery(event: TraceEvent, query: string): boolean {
  const q = query.toLowerCase();
  if (event.summary.toLowerCase().includes(q)) return true;
  if (event.type.toLowerCase().includes(q)) return true;
  if (event.source.toLowerCase().includes(q)) return true;
 ...
 */
export function createTraceSearchEffect(store: Store): Unsubscribe {
  return store.subscribe<string>(TOPICS.TRACE_QUERY, (query) => {
    const trimmed = query.trim();
    if (trimmed.length === 0) return;

    defer(() => {
      const dataPayload = store.get<TopicPayload<TraceEvent[]>>(TOPICS.TRACE_DATA);
      const events = dataPayload?.data ?? [];

      const results = events.filter((e) => matchesQuery(e, trimmed));
      results.sort((a, b) => a.timestamp.localeCompare(b.timestamp));

      const payload: TopicPayload<TraceEvent[]> = { status: "ok", data: results };
