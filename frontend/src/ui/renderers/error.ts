 ...
 */
export function renderSimpleError(message: string): string {
...
function renderFieldErrors(
  ...
  const items = entries
    .map(
      ([field, msg]) =>
        `<li><strong>${escapeHtml(field)}</strong>: ${escapeHtml(msg)}</li>`,
    )
    .join("");
