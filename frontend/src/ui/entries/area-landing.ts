import type { FeatureArea } from "../../capabilities/types";
 ...
 */
export function initAreaLanding(area: FeatureArea): () => void {
  ...
  const capabilities = getByArea(area) as CapabilityShell[];
  ...
  unsubs.push(
    store.subscribe<TopicPayload<CapabilityShell[]>>(TOPICS.CAPABILITIES, (payload) => {
      if (payload.status !== "ok" || !payload.data || !cardsEl) return;

      // Only apply if we haven't received resolved capabilities yet
      const resolved = store.get<TopicPayload<ResolvedCapability[]>>(TOPICS.RESOLVED_CAPABILITIES);
