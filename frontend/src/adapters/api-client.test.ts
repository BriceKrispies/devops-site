...
import type { ApiErrorResponse, NormalizedError } from "../types/api-error";
...
function test_validation_error_normalizes_correctly(): void {
  ...
  const result: NormalizedError = normalizeApiError(apiError, 400);
...
function test_not_found_error_normalizes_correctly(): void {
  const apiError: ApiErrorResponse = {
    code: "NOT_FOUND",
...
function test_unauthenticated_error_normalizes_to_permission(): void {
  const apiError: ApiErrorResponse = {
...
function test_forbidden_error_normalizes_to_permission(): void {
  const apiError: ApiErrorResponse = {
    code: "FORBIDDEN",
...
function test_dependency_unavailable_normalizes_to_unavailable(): void {
  const apiError: ApiErrorResponse = {
...
function test_internal_error_normalizes_to_unexpected(): void {
  const apiError: ApiErrorResponse = {
    code: "INTERNAL_ERROR",
...
function test_conflict_error_normalizes_correctly(): void {
  const apiError: ApiErrorResponse = {
    code: "CONFLICT",
...
function test_rate_limited_normalizes_correctly(): void {
  const apiError: ApiErrorResponse = {
    code: "RATE_LIMITED",
...
function test_kill_switch_normalizes_to_unavailable(): void {
  const apiError: ApiErrorResponse = {
...
function test_unknown_code_falls_back_to_http_status(): void {
  const apiError: ApiErrorResponse = {
...
function test_no_field_errors_results_in_null(): void {
  const apiError: ApiErrorResponse = {
    code: "VALIDATION",
    message: "Bad input.",
